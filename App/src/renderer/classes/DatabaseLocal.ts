...
import {
    ...
    IGoalItem,
    IGoalNestedItems, IGoalNestedItemWithCommentAndReminder, TaskItem
...
import SqlLoader from '@/classes/SqlLoader';

const Database = require( 'better-sqlite3' );

export default class DatabaseLocal implements IZDatabase {
    ...
    public config: ConfigStore = new ConfigStore( DatabaseLocal.configFileName, DatabaseLocal.databaseDirectoryName );

    public sqlLoader: SqlLoader = new SqlLoader();

    constructor() {
        let existingData = this.config.get( DatabaseLocal.localDataKey );
        if ( !existingData ) {
            this.config.set( DatabaseLocal.localDataKey,
                {
                    databaseList: [],
                    lastOpenedDatabase: ''
                }
    ...
    public getConfigData(): { databaseList: { src: string, date: number }[], lastOpenedDatabase: string } {
        let configData = this.config.get( DatabaseLocal.localDataKey );
        if ( configData ) {
            return configData;
        }
        return {
    ...
    public setLastOpenedDatabasePath( path: string ) {
        ...
        if ( configData ) {
            ...
            this.config.set( DatabaseLocal.localDataKey, configData );
        } else {
    ...
    fetchTasks( options: ArgFetchTasks ): IGoalNestedItems {
        ...
        if ( options.description ) {
            ...
            stmt = this.sqlLoader.prepareSqlFor( 'tasks-search', options, this.db );
        } else {
            stmt = this.sqlLoader.prepareSqlFor( 'tasks', options, this.db );
        }
        let all: IGoalNestedItems = stmt.all( options )
        all.forEach( ( val ) => {
    ...
    addTask( nestedItem: IArgAddTask ): IArgAddNested | false {
        ...
        if ( !!result.changes ) {
            ...
            let items: IGoalChangeItemsOrder = selectStmt.all( {
                parentId: nestedItem.parentId
            } );
            let newItemsOrder: IGoalChangeItemsOrder = [];
            let insertIndex: number = 0;
            for ( let i = 0; i < items.length; i++ ) {
                if ( items[ i ].id == result.lastInsertRowid ) {
                    insertIndex = i;
                }
                newItemsOrder.push( {
                    order_key: i,
    ...
    fetchTaskById( id: number ): IGoalNestedItemWithCommentAndReminder {
        let query = 'SELECT ci.*,\n' +
            '       c.text          AS item_comment_text,\n' +
            '       c.id            AS item_comment_id,\n' +
            '       c.date_creation AS item_comment_date_creation,\n' +
            ...
            'WHERE ci.id = @id';
        const stmt = this.db.prepare( query );
        let all: IGoalNestedItems = stmt.all( { id } );
        all.forEach( ( val ) => {
            val.labels = this.fetchLabelsForNestedItemById( val.id );
        } );
        if ( all.length > 0 ) {
